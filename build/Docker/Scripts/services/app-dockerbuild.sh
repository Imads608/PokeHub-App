...
docker build --rm --target final --build-arg "TAG=${tag}" -t app-base-image:${tag} -f build/Docker/Dockerfiles/app-Dockerfile .
...
echo "Done building App Base Docker Image with tag ${tag}"
