...
import {
  getDM,
  getDMWithConversation,
  getChatRooms as getChatRoomsAPI,
  joinChatRoom as joinChatRoomAPI,
  ...
  try {
    ...
    dispatch(
      setErrorNotification(
        'Unable to load conversation. Please refresh and try again',
        'DM',
        LOAD_DM_CONVERSATION,
        { id: dmId }
      )
...
export const loadChatRoomConversation = (roomId) => async (dispatch) => {
  try {
    ...
    dispatch(
      setErrorNotification(
        'Unable to load conversation. Please refresh the page and try again',
        'ChatRoom',
        LOAD_CHATROOM_CONVERSATION,
        { id: roomId }
      )
...
export const loadUserDM =
  (participants, browserHistory, isLink, viewType) => async (dispatch) => {
    ...
    try {
      ...
      dispatch(
        setErrorNotification(
          err.message,
          viewType === 'LINK' ? 'NewDM_LINK' : 'NewDM_POPPER',
          SET_DM_ACTIVE
        )
      );
    }
...
export const loadUserDMWithConversation = (dmId, user) => async (dispatch) => {
  try {
    ...
    dispatch(
      setErrorNotification(DM_LOAD_ERROR, 'AlertNotification', SET_DM_ACTIVE)
    );
  }
};
