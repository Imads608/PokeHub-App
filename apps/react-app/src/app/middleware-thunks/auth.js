...
import { loggedIn, authFailure, resetAuthError, loggedOut, setAuthLoaded } from '../actions/auth';
import { appLoaded } from '../actions/app';
import { getRequestHeaderWithToken, getAPIRequestHeader, setAuthToken } from '../utils';
...
const loadUserDataAndDispatchLoginState = async (dispatch, user) => {
    ...
    try {
        ...
        dispatch(loggedIn({
            token: token,
            user: {
                uid, name: displayName, email, emailVerified, username: userServiceRes.username, 
                first_name: userServiceRes.first_name, last_name: userServiceRes.last_name, joinedRooms: userServiceRes.joinedRooms 
            }
        }))
    } catch (err) {
        ...
        if (err.response && err.response.status === 400) {
            ...
            await createUserDataAndDispatchLoginState(dispatch, user, tempUsername, user.email)
...
const createUserDataAndDispatchLoginState = async (dispatch, user, username, email) => {
    try {
        const token = await user.getIdToken();
        ...
        dispatch(loggedIn({
            token: token,
            user: {
                uid: user.uid, name: user.displayName, email, emailVerified: user.emailVerified, 
                username: userServiceRes.username, first_name: userServiceRes.first_name, last_name: userServiceRes.last_name,
...
export const createUser = (username, email, password) => async dispatch => {
    try {
        ...
        const fbAuthRes = await auth.createUserWithEmailAndPassword(email, password);
        console.log('Got result from creating user', fbAuthRes);
        shouldCreateUser = false;
        await createUserDataAndDispatchLoginState(dispatch, fbAuthRes.user, username, email);
    } catch (err) {
