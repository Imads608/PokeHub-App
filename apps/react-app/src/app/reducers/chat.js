import { CHAT_OPENED, CHAT_CLOSED, GET_CHAT_ROOMS_SUCCESS, SET_CHATROOM_STATE, LOAD_CHATROOM_CONVERSATION, DM_SENT, DM_RECEIVED, 
         RESET_DM_UNREAD_MESSAGES, CHATROOM_MESSAGE_SENT, CHATROOM_MESSAGE_RECEIVED, SET_DM_ACTIVE, SET_DM_INACTIVE, OPEN_DM,
         CLOSE_DM, CLOSE_ALL_DMS, LOAD_DM_CONVERSATION } from '../actions/types/chat';
    ...
    builder
        ...
        .addCase(DM_SENT, (state, action) => {
            state.activeDMs = state.activeDMs.map(dm => {
                if (dm.id === action.payload.roomId && !dm.state.messages.find(message => message.id === action.payload.id)) {
        ...
        .addCase(DM_RECEIVED, (state, action) => {
            if (!state.activeDMs.find(dm => dm.id === action.payload.dm.id)) {
                state.activeDMs.push({ ...action.payload.dm, state: { messages: [ action.payload.message ], unread: 1, isOpened: false, isConversationLoaded: true, viewType: VIEW_TYPE_POPPER }})
            } else {
                state.activeDMs.map(dm => {
                    if (dm.id === action.payload.dm.id && !dm.state.messages.find(message => message.id === action.payload.message.id)) {
        ...
        .addCase(SET_DM_ACTIVE, (state, action) => {
            if (!state.activeDMs.find(dm => dm.id === action.payload.id)) {
                state.activeDMs.push({ ...action.payload, state: { ...action.payload.state, isOpened: true }});
            } else {
                state.activeDMs.map(dm => {
        ...
        .addMatcher((action) => action.type === CHATROOM_MESSAGE_SENT || action.type === CHATROOM_MESSAGE_RECEIVED, (state, action) => {
            state.publicRooms = state.publicRooms.map(room => {
