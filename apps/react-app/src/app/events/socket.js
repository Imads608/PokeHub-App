...
export const connectWebSocket = (action) => {
    ...
    socket.on("connect", () => {
        ...
        const messageEvent = getMessageEvent(userEvents.CLIENT_DETAILS,
                                            { uid: action.payload.user.uid, username: action.payload.user.username },
                                            getUserClientConnectedEvent({ uid: action.payload.user.uid, username: action.payload.username,
                                                                          socketClient: socket.id}, { publicRooms: action.payload.joinedPublicRooms,
                                                                                                      privateRooms: null }));
