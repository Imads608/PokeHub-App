...
export const loginUser = async (userId, password) => {
    ...
    try {
        const resp = await axios.post(`${appConfig.apiGateway}/auth/login`, { email: userId, password });
        return resp.data;
    } catch (err) {
        //if (err.message)
...
export const signupUser = async (email, username, password) => {
    ...
    const resp = await axios.post(`${appConfig.apiGateway}/users`, { email, username, password }, getAPIRequestHeader());
    return resp.data;
}

export const getNewAccessToken = async (refreshToken) => {
    const resp = await axios.get(`${appConfig.apiGateway}/auth/access-token`, { headers: { Authorization: refreshToken }});
    return resp.data.access_token;
}

export const loadUser = async (accessToken) => {
    const resp = await axios.get(`${appConfig.apiGateway}/users/auth`, { headers: { Authorization: accessToken }});
    return resp.data;
}

export const googleOAuthLogin = async (googleTokenId) => {
    console.log('Logging in Google OAuth User', googleTokenId);
    const response = await axios.post(`${appConfig.apiGateway}/auth/oauth-google`, null, { headers: {
