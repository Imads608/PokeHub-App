...
export const getDM = (participants, viewType) =>
...
export const getDMFromID = (dmId, user, viewType) =>
  new Promise((resolve, reject) =>
    setTimeout(() => {
      const result = {
        id: uuidv4(),
        ...
        name: '',
        isActive: true,
        participants: [user, membersSeed[0]],
        state: {
          messages: [],
          unread: 0,
          isConversationLoaded: false,
...
export const getDMWithConversation = (id, user, viewType = 'NewDM_POPPER') =>
  new Promise((resolve, reject) =>
    setTimeout(() => {
      const result = {
        id,
...
export const getChatRoomMembers = (offset, limit, search) =>
  new Promise((resolve, reject) =>
    setTimeout(() => {
      console.log('Search', search);
      let members = membersSeed;
      if (search) {
        members = membersSeed.filter((member) =>
      ...
      if (offset + limit >= members.length)
        resolve({ data: result, isDone: true });
...
export const getChatRooms = async () => {
  const chatrooms = await axios.get(
    `${appConfig.apiGateway}/chat/public-rooms`
  );
...
export const joinChatRoom = async (roomId, uid) => {
  ...
  const resp = await axios.post(
    `${appConfig.apiGateway}/chatrooms/${roomId}/members`,
    body,
    getAPIRequestHeader()
  );
  return resp.data;
...
export const leaveChatRoom = async (roomId, uid) => {
  const resp = await axios.delete(
    `${appConfig.apiGateway}/chatrooms/${roomId}/members/${uid}`,
    getAPIRequestHeader()
  );
  return resp.data;
};
