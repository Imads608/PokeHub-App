...
const getHighlightedText = (text, highlight) => {
    ...
    return <span> { parts.map((part, i) => 
        <span key={i} style={part.toLowerCase() === highlight.toLowerCase() ? { backgroundColor: 'pink' } : {} }>
            { part }
        </span>)
    } </span>;
}
            ...
            <Hidden mdDown>
                { location.pathname.includes('/dms') ? (
                    <ListItem button onClick={() => loadDM(searchedUser, true) } className='search-result'>
                        ...
                        <span className='lead'>{getHighlightedText(searchedUser.username, searchTerm)}</span>
                    </ListItem>
                ) : (
                    <ListItem button onClick={() => loadDM(searchedUser, false)} className='search-result'>
                        <Avatar className='search-result-avatar' src='/broken-image.jpg' />
                        <span className='lead'>{getHighlightedText(searchedUser.username, searchTerm)}</span>
                    </ListItem>
                )}
