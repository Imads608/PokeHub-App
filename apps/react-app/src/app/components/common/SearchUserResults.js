...
const SearchUserResults = ({ loadingDM, fetched: { results, localLoading, offset, search, isDone }, fetched, getResults, loadDM, error, resetError }) => {
    return (
        localLoading || loadingDM ? <CircularProgress style={{ margin: 'auto' }} color='secondary' /> : 
        error ? (
            <section className='empty-search-results'>
                <div style={{ fontSize: '20px' }} className='drawer-header'>Uh Oh. Something went wrong</div>
                <SentimentVeryDissatisfiedIcon fontSize='large' style={{ width: '80px', height: '80px', color: 'grey' }} />
                <Button className='theme-text' variant='contained' color='secondary' onClick={resetError}>
                    Go Back
                </Button>
            </section>
        ) :
        search === '' ? (
            <section className='empty-search-results'>
                <SearchIcon fontSize='large' style={{ width: '80px', height: '80px', color: 'grey' }} />
                <div style={{ fontSize: '20px' }} className='drawer-header'>Start Searching!</div>
            </section>
        ...
        (
            <List id='scrollUsers' className='search-results'>
                ...
                >
                    {fetched && results.map((searched) => (
                        <SearchedUser searchedUser={searched} key={searched.uid} loadDM={loadDM} searchTerm={search} />
                    ))}
                </InfiniteScroll>
            </List>
        )
