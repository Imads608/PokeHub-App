...
const Pokedex = ({ location, history }) => {
  ...
  useEffect(() => {
    location.pathname === '/dex/pokemon' &&
      navSelected !== 'pokemon' &&
      setNavSelected('pokemon');
    location.pathname === '/dex' &&
      navSelected !== 'search' &&
      setNavSelected('search');
    location.pathname === '/dex/moves' &&
      navSelected !== 'moves' &&
      setNavSelected('moves');
    location.pathname === '/dex/abilities' &&
      navSelected !== 'abilities' &&
      setNavSelected('abilities');
    location.pathname === '/dex/types' &&
      navSelected !== 'types' &&
      setNavSelected('types');
  }, [location.pathname]);

  const handleChange = (event, newValue) => {
    newValue !== 'search'
      ? history.push(`/dex/${newValue}`)
      : history.push('/dex');
  ...
  return isLoading ? (
    ...
    <div style={{ height: '93vh', display: 'flex', flexDirection: 'column' }}>
      ...
      >
        <div
          className="dex-search-nav-item"
          style={{}}
          onClick={(e) => handleChange(e, 'search')}
        >
      ...
      <Slide
        direction="right"
        in={location.pathname === '/dex/pokemon'}
        mountOnEnter
        unmountOnExit
      >
        <div
      ...
      <Slide
        direction="right"
        in={
          location.pathname.includes('/dex/pokemon/') &&
      ...
      >
        ...
        >
          <Pokemon
            dexEntry={
              selectedEntry.type && selectedEntry.type == 'pokemon'
                ? selectedEntry.entry
                : null
            }
            location={location}
