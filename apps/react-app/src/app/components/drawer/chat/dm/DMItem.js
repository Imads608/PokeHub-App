...
const DMLink = ({ currentDM, dm, setDMtoInactive, classes, openDM, username }) => {
    return (
        <ListItem
            button
            component={Link}
            ...
            onClick={openDM}
        >
            <CancelIcon fontSize="small" className={classes.closeIcon} onClick={setDMtoInactive} />
            <Badge badgeContent={dm.state.unread} color='primary'>
                <Avatar style={{ marginRight: '10px' }} className={classes.small} src='/broken-image.jpg' />
            </Badge>
            <ListItemText
                primary={
                    <h5 className={`drawer-header-text drawer-expanded-list-item-text drawer-header`}>{username === dm.participants[1].username ? dm.participants[0].username : dm.participants[1].username}</h5>
                } 
            />
        </ListItem>
    )
...
const DMNonLink = ({ currentDM, dm, setDMtoInactive, classes, openDM, username }) => {
    return (
        <ListItem 
            button
            selected={currentDM && dm.id === currentDM.id}
...
const DMItem = ({ dm, openedDM, currentDM, closedDM, setDMInactive, username }) => {
...
export default connect(mapStateToProps, { openedDM, closedDM, setDMInactive })(DMItem);