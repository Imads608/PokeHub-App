...
const DMs = ({ dmsToggle, openedDMs, closedDMs, activeDMs, currentDM, drawerRef, closedDM, unreadTotalCount, drawerToggle }) => {
    ...
    useEffect(() => {
        ...
        currentDM && currentDM.state.viewType === VIEW_TYPE_LINK && !location.pathname.includes('/dms') && !location.pathname.includes(currentDM.id) && closedDM(currentDM.id);
    ...
    const startNewDM = (e) => {
        currentDM && currentDM.state.viewType === VIEW_TYPE_POPPER && closedDM(currentDM.id);
    ...
    return (
        <React.Fragment>
                ...
                >
                    <List className={classes.headerBlock}>
                        <Badge badgeContent={unreadTotalCount} color='primary'>
                            <ChatIcon fontSize='large' className={`drawer-icon ${dmsToggle ? 'drawer-header-expanded' : 'drawer-header'}`} />
                        </Badge>
                        <ListItemText primary={<h4 className={`drawer-header-text ${dmsToggle ? 'drawer-header-expanded' : 'drawer-header'}`}>DMs</h4>} />
                    </List>
                </AccordionSummary>
                <AccordionDetails className='drawer-accordion-details'>
                    <List className='drawer-expanded-list'>
                        <NewDMToggle toggleNewDM={toggleNewDM} startNewDM={startNewDM} location={location} />
                        {activeDMs && activeDMs.map((dm, index) => (
            ...
            <Hidden mdDown>
                ...
                >
                    {({ TransitionProps }) => (
                        <Grow mountOnEnter unmountOnExit {...TransitionProps} in={toggleNewDM} timeout={500}>
                            { toggleNewDM ? <div ref={childPopper}><NewDM /></div> : <div></div> }
                        </Grow>
                    )}
                </Popper>
                <Popper 
                ...
                >
                    {({ TransitionProps }) => (
                        <Grow mountOnEnter unmountOnExit {...TransitionProps} in={currentDM !== null} timeout={500}>
                            { currentDM ? <div ref={childPopper}><DM /></div> : <div></div> }
                        </Grow>
                    )}
                </Popper>
            </Hidden>
...
export default connect(mapStateToProps, { openedDMs, closedDMs, closedDM })(DMs);