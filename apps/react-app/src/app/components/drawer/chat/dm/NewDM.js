...
const NewDM = ({ fetched, getResults, resetFilter, searchOnNewFilter, publicUser }) => {
    const classes = useStyles();

    const [startDM, setStartDM] = useState({ participants: null, enable: false });
    const { data: dmData, error: dmError, isLoading: dmLoading, remove } = useDMLoadFromParticipants(startDM.participants, VIEW_TYPE_POPPER, startDM.enable);
...
export default connect(mapStateToProps)(WithResultsSearch(NewDM, getChatRoomMembers, null, NEW_DM));