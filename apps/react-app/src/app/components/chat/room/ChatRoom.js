...
const ChatRoom = ({
  ...
  useEffect(() => {
    chatrooms &&
      (!currentWindow.payload ||
        currentWindow.type !== CHAT_ROOM ||
        (currentWindow.payload &&
          currentWindow.payload.id !== parseInt(match.params.id))) &&
      setChatRoomOpen();
  ...
  setChatRoomOpen = () => {
    const currentChat = chatrooms.find(
      (room) => room.id === parseInt(match.params.id)
    );
    if (currentChat) {
      openedChatRoom(
      ...
      setOpenWindow({
        type: CHAT_ROOM,
        payload: { id: currentChat.id, name: currentChat.name },
      });
    } else
      setErrorNotification(CHAT_ROOM_DOESNT_EXIST, 'AlertNotification', null);
  };

  return !isLoading &&
    !currentRoom &&
    notificationMessage === CHAT_ROOM_DOESNT_EXIST ? (
    ...
    <Grid container spacing={0}>
      ...
      <Grid item xs={12} md={9}>
        <div
          style={{ display: 'flex', height: '93vh', flexDirection: 'column' }}
        >
          <Hidden mdUp>
            <ChatMobileHeader
          ...
          {location.search === '?view=rules' &&
          parseInt(match.params.id) === currentRoom.id ? (
            <Rules location={location} room={currentRoom} />
          ) : location.search === '?view=lobby' &&
            parseInt(match.params.id) === currentWindow.payload.id ? (
            <Lobby
              user={user}
              isMember={isMember}
              room={currentRoom}
            ...
            <Members
              ...
              searchOnNewFilter={searchOnNewFilter}
            />
          ) : location.search === '' &&
            parseInt(match.params.id) === currentWindow.payload.id ? (
...
export default connect(mapStateToProps, {
  ...
  withWidth()(
    WithResultsSearch(ChatRoom, getChatRoomMembers, null, TYPE_CHAT_CHATROOM)
  )
);
