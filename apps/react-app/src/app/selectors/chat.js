...
const activeChatrooms = state => state.chat.publicRooms && state.chat.publicRooms.filter(opened => opened.state.isActive === true)
const openedChatroom = state => state.chat.publicRooms && state.chat.publicRooms.find(room => room.state.isOpened === true)
...
export const getActiveDMs = createSelector(
    [activeDirectMessages], activeDMs => activeDMs
);

export const getOpenedDM = createSelector(
    [getActiveDMs], (activeDMs) => {
...
export const getOpenedDMRecipients = createSelector(
    [getOpenedDM], currentDM => currentDM && currentDM.participants.slice(1)
);

export const getUnreadDMs = createSelector(
    [getActiveDMs], activeDMs => (
