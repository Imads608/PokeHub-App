...
export class AuthController {
  ...
  emailLogin(user: EmailLogin): Promise<UserDataWithToken> {
    this.logger.log(
      `emailLogin: Got request to login user with Email ${user.email}`
    );
  ...
  usernameLogin(user: UsernameLogin): Promise<UserDataWithToken> {
    this.logger.log(
      `usernameLogin: Got request to login user by Username ${user.username}`
    );
  ...
  googleOAuthLogin(token: string) {
    this.logger.log(
      `googleOAuthLogin: Got request to login user with Google OAuth`
    );
  ...
  validateAccessToken(accessToken: string): Promise<boolean> {
    this.logger.log(
      'validateAccessToken: Got request to validate provided access token'
    );
  ...
  validateEmailConfirmationToken(accessToken: string): Promise<JwtTokenBody> {
    this.logger.log(
      'validateEmailConfirmationToken: Got request to validate provided Email Verification Token'
    );
  ...
  generateTokens(userInfo: JwtTokenBody): Promise<AuthTokens> {
    this.logger.log(
      `generateTokens: Got request to generate tokens for user with uid ${userInfo.uid}`
    );
  ...
  getEmailVerificationToken(
    ...
    this.logger.log(
      'getEmailVerificationToken: Got request to generate new Email Verification Token'
    );
