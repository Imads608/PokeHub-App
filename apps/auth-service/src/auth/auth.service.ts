...
export class AuthService implements IAuthService {
    ...
    constructor(@Inject("UserMicroservice") private readonly clientProxy: ClientProxy,
                private readonly jwtService: JwtService, private configService: ConfigService, private readonly logger: AppLogger) {
        this.client = new OAuth2Client(configService.get<string>('googleClientCreds.id'), configService.get<string>('googleClientCreds.secret'));
    ...
    async googleOAuthLogin(tokenId: string): Promise<UserDataWithToken> {
        try {
            ...
            const userData = await firstValueFrom(this.clientProxy.send<IUserData>({ cmd: TCPEndpoints.GOOGLE_OAUTH_LOGIN }, createReq));
            if (!userData)
    ...
    async getNewEmailVerificationToken(user: JwtTokenBody ): Promise<{ email_verification_token: string }> {
        try {
            ...
            const token = this.jwtService.sign(user, {
                secret: this.configService.get<string>('ACCESS_TOKEN_SECRET'),
                expiresIn: `${this.configService.get<string>('EMAIL_VERIFICATION_TOKEN_EXPIRATON_SECONDS')}s`
            });
    ...
    async emailLogin(userCreds: EmailLogin): Promise<UserDataWithToken> {
        try {
            ...
            const user: UserData = await firstValueFrom(this.clientProxy.send<UserData>({ cmd: TCPEndpoints.FIND_USER_EMAIL }, userCreds.email));
            if (!user)
                throw new RpcException('Invalid Credentials');

            this.logger.log(`emailLogin: Successfully retrieved User Data from User Microservice with email: ${user.email}`);
            ...
            if (isValidated) {
                ...
                return new UserDataWithToken(user, tokens.accessToken, tokens.refreshToken);
            }
            throw new RpcException("Invalid Credentials");
        } catch (err) {
    ...
    async usernameLogin(userCreds: UsernameLogin): Promise<UserDataWithToken> {
        try {
            ...
            const user: UserData = await firstValueFrom(this.clientProxy.send<IUserData>({ cmd: TCPEndpoints.FIND_USER_USERNAME }, userCreds.username));
            if (!user)
                throw new RpcException('Invalid Credentials');

            this.logger.log(`usernameLogin: Successfully retrieved User Data from User Microservice with username: ${user.username}`);
            ...
            if (isValidated) {
                const tokens = await this.generateNewTokens(new JwtTokenBody(user.username, user.email, user.uid));
                return new UserDataWithToken(user as IUserData, tokens.accessToken, tokens.refreshToken);
            }
            throw new RpcException("Invalid Credentials");
        } catch (err) {
