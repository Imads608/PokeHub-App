...
export class MailService implements IMailService {
    ...
    async sendEmailConfirmation(emailAddress: string, validationToken: string): Promise<void> {
        try {
            ...
            await firstValueFrom(this.clientProxy.send({ cmd: TCPEndpoints.SEND_EMAIL_CONFIRMATION }, 
                new EmailConfirmationRequest(emailAddress, validationToken))).catch(err => {
                    if (!err.message.includes("no elements in sequence"))
                        throw err;
                });
        } catch (err) {
            this.logger.error(`sendEmailConfirmation: Got error while sending email confirmation request to ${emailAddress}: ${err}`);
            throw err;
        }
    }
}
