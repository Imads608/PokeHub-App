...
export class MessageReceiverService {
  ...
  async userStatusEventMessageHandler( msg: UserEventMessage<UserStatusEvent>, amqpMsg: ConsumeMessage ): Promise<void> {
    try {
      this.logger.log(`userStatusEventMessageHandler: Got message to process User Status for id ${msg.data.status.id} with timestamp ${JSON.stringify(amqpMsg.properties)}`);
      ...
      this.logger.log(`userStatusEventMessageHandler: Successfully updated User Status for id ${msg.data.status.id} with timestamp ${amqpMsg.properties.timestamp}`);
    } catch (err) {
