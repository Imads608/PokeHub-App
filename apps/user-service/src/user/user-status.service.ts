...
export class UserStatusService implements IUserStatusService { 
    ...
    async getLastSeenOfUser(userId: string): Promise<IUserStatusData> {
        try {
            ...
            const data = new UserStatusData(userStatus.uid, userStatus.status, userStatus.lastSeen);
            this.logger.log(`getLastSeenOfUser: Successfully fetched Last Seen Data of user ${userId}: ${JSON.stringify(data)}`);
            return data;
        } catch (err) {
            this.logger.error(`getLastSeenOfUser: Got error getting last seen data of user ${userId}: ${err}`);
            throw err;
        }
    }
}
