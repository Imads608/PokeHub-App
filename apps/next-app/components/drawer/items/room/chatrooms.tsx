            ...
            <Accordion expanded={chatroomsMenuToggle} onChange={() => toggleChatRoomsExpand()}>
                ...
                >
                    <List className={classes.headerBlock}>
                        ...
                        <ListItemText primary={<h4 className={`${styles['drawer-header-text']} ${chatroomsMenuToggle ? styles['drawer-header-expanded'] : styles['drawer-header']}`}>Chat Rooms</h4>} />
                    </List>
                </AccordionSummary>
                            ...
                            <Link key={opened.id} href={{ pathname: opened.state.url.pathname, query: opened.state.url.query }} passHref>
                                <ListItemButton
                                    ...
                                    <ListItemText 
                                        primary={
                                            <h5 
                                            className={`${styles['drawer-header-text']} ${styles['drawer-expanded-list-item-text']} 
                                            ${currentWindow.type === WindowTypes.CHAT_ROOM && currentWindow.payload && currentWindow.payload.id === opened.id ? 
                                            styles['drawer-header-expanded'] : styles['drawer-header']}`}
                                            ># {opened.name}</h5>} 
                                    />
                                </ListItemButton>
                            </Link>
                                ...
                                {chatrooms && chatrooms.map((room, id) => (
                                    ...
                                    (
                                        <Link href={`/chatrooms/${room.id}`}>
                                            <MenuItem className='drawer-header' key={id} onClick={closeMenu}>#{room.name}</MenuItem>
                                        </Link>
                                    )
                                ))}
                            </Menu>
