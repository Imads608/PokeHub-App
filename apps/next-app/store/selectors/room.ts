...
const activeChatrooms = (state: RootState) =>
  state['room-state'].publicRooms &&
  state['room-state'].publicRooms.filter(
    (room) => room.state.isActive === true
  );
const openedChatroom = (state: RootState) =>
  state['room-state'].publicRooms &&
  state['room-state'].publicRooms.find((room) => room.state.isOpened === true);
