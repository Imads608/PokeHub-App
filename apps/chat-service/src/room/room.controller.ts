...
export class RoomController {
    ...
    constructor(@Inject(ROOM_SERVICE) private roomService: IRoomService, private readonly logger: AppLogger) {
    ...
    joinChatroom(userId: string, roomId: string): Promise<Participant> {
        this.logger.log(`joinChatRoom: Got Request for user ${userId} to join chatroom ${roomId}`);
    ...
    getUserChatrooms(userId: string): Promise<ChatRoom[]> {
        this.logger.log(`getUserChatrooms: Got request to fetch all Chatrooms User ${userId} has joined`);
    ...
    getPublicChatrooms(): Promise<ChatRoom[]> {
        this.logger.log(`getPublicChatrooms: Got request to fetch all Public Chatrooms`);
    ...
    getPublicChatRoom(roomId: string): Promise<ChatRoom> {
        this.logger.log(`getPublicChatroom: Got request to fetch data related to chatroom ${roomId}`);
        return this.roomService.getPublicRoomById(roomId);
    }

    @MessagePattern({ cmd: TCPEndpoints.GET_PUBLIC_CHATROOM_USERS }, Transport.TCP)
    getPublicChatRoomUsers(roomId: string): Promise<Participant[]> {
        this.logger.log(`getPublicChatRoomUsers: Got request to retrieve all Participants of Chatroom ${roomId}`);
