    ...
    async sendEmailConfirmation(recipient: EmailConfirmationRequest): Promise<void> {
        await this.mailerService.sendMail({
            ...
            html: `
                ...
                <a target="_" href="${this.configService.get<string>('frontendDetails.host')}:
                ${this.configService.get<string>('frontendDetails.port')}${this.configService.get<string>('emailVerificationEndpoint')}?activation_token=${recipient.linkValidationToken}"
            `
        });
    }
}
