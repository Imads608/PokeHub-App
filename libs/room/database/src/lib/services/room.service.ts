...
export class RoomService implements IRoomService {
  constructor(@InjectRepository(ChatRoom) private chatRoomRepository: Repository<ChatRoom>, 
    @InjectRepository(Participant) private participantRepository: Repository<Participant>, private readonly logger: AppLogger) {
    logger.setContext(RoomService.name);
  }

  async getAllPublicRooms(): Promise<ChatRoom[] | null> {
  ...
  async getUserJoinedRooms(userId: string): Promise<ChatRoom[] | null> {
  ...
  async getChatRoomParticipants(roomId: string): Promise<Participant[] | null> {
