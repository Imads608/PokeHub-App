...
export const getUser = createSelector([user], (userData) => userData);

export const getUsername = createSelector([userName], (username) => username);

export const getUid = createSelector([userId], (uid) => uid);
...
export const isUserMemberOfCurrentChatRoom = createSelector(
  ...
  (joinedRooms, currentWindow) =>
    ...
    joinedRooms.find((room) => room.id === currentWindow.payload.id)
      ? true
      : false
);
