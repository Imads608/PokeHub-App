import { Chip, Grid, Hidden, ListItem } from '@mui/material';
    ...
    >
      {data.map((dexEntry, i) => (
        <ListItem
          component={Link}
          button
          to={`/dex/pokemon/${dexEntry.id}`}
          onClick={() => setSelectedEntry({ type: 'pokemon', entry: dexEntry })}
          className={`results-row ${i % 2 !== 0 ? 'odd-results-row' : ''}`}
          key={i}
        >
          <Grid item xs={1} md={1}>
            <div className="theme-text-whitebkg"># {dexEntry.id}</div>
          </Grid>
          <Grid item xs={4} md={2}>
            <div
              className="overworld-sprite"
              style={{
                background: `transparent url(https://play.pokemonshowdown.com/sprites/pokemonicons-sheet.png?v5) no-repeat scroll ${getWidth(
                  dexEntry.id
                )}px ${getHeight(dexEntry.id)}px`,
              }}
            ></div>
          ...
          <Hidden mdDown>
            <Grid container item md={6} xs={false}>
              {['HP', 'Attack', 'Defense', 'SpAtt', 'SpDef', 'Speed'].map(
                (statObj, index) => (
      ...
      <div
        ...
        style={{ marginLeft: '10px', fontStyle: 'large' }}
      >
        (
        {hasNextPage && bottomReached ? 'Fetching...' : 'No more data to Load!'}
      </div>
    </Grid>
