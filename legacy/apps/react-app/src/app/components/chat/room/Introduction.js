...
const Introduction = ({
  ...
  return (
    <div className="theme-text-whitebkg intro-window main-window">
      <div className="title-format header">{`Welcome to # ${room.name}`}</div>
      <div className="theme-text-whitebkg">{room.description}</div>
      <div
        style={{
          marginTop: '10px',
          display: 'flex',
      ...
      >
        ...
        {user.joinedRooms.find((publicRoom) => publicRoom.id === room.id) ? (
          <Button
            ...
            onClick={() => leaveChatRoom(room.id, user.uid)}
            color="secondary"
            variant="contained"
          >
            Leave Room
          ...
          <Button
            style={{ fontFamily: 'Orbitron' }}
            onClick={() => joinChatRoom(room.id, user.uid)}
            color="secondary"
            variant="contained"
          >
            + Join Room
...
export default connect(null, { joinChatRoom, leaveChatRoom, setChatRoomState })(
  Introduction
);
