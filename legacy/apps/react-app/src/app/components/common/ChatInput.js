...
const ChatInput = ({
  recipient,
  publicUser: { username, uid },
  sentMessageToRecipient,
  typeChat,
  disabled,
}) => {
  ...
  useEffect(() => {
    ...
    socket.on(USER_IS_TYPING, (msg) => {
      ...
      if (
        ...
        ((typeChat === TYPE_CHAT_DM && msg.room === recipient.roomId) ||
          (typeChat === TYPE_CHAT_CHATROOM && recipient.name === msg.room))
      ) {
    ...
    socket.on(USER_STOPPED_TYPING, (msg) => {
      ...
      if (
        ...
        setOtherUsersTyping(
          otherUsersTyping.filter((username) => username !== msg.username)
        );
      }
    });
  }, []);
  ...
  const handleTyping = () => {
    ...
    socket.emit(USER_STOPPED_TYPING, {
      ...
      username,
      to: typeChat === TYPE_CHAT_DM ? recipient.roomId : recipient.name,
      room: typeChat === TYPE_CHAT_DM ? recipient.roomId : recipient.name,
    });
  };
  ...
  const placeholder =
    typeChat === TYPE_CHAT_DM && recipient.participants.length === 2
      ? `Message @${recipient.participants[1].username}`
      ...
      {otherUsersTyping.length > 0 ? (
        <div style={{ marginLeft: '15px', fontSize: 'small' }}>
          {otherUsersTyping.length === 1
            ...
            : `${otherUsersTyping[0]}, ${otherUsersTyping[1]} and ${
                otherUsersTyping.length - 2
              } are typing`}
        </div>
      ) : (
